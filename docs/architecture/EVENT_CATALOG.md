...
| **`autonomy.message_sent`** ⚠ (010, Constitution Amendment 1.2.0) | conversation_id, message_id, pending_autonomous_send_id, generation_id, category — **never** the sent message's own body (already covered by `ai.draft_generated`/`ai.draft_accepted` at generation/composition time). `actor_type = "SYSTEM"` — see `booking_script.autonomous_message_sent`'s own note above for the exhaustive-query pattern covering both. |
