...
| **`scheduling.availability_bootstrap_seeded`** (012/AC-1) | wide_slots_created, specialty_count, business_day_count, generalist_d1_created, generalist_d7_created — `actor_type = "SYSTEM"`; emitted once per `run_bootstrap_seed()` call (the env-gated app-startup hook or `python -m customer_care.scheduling.bootstrap_seed`). Distinct from `scheduling.wide_availability_seeded` / `scheduling.availability_seeded` so an auditor can tell "seeded at stack startup" from "an operator clicked a button". Not conversation-scoped |
| **`scheduling.availability_floor_topped_up`** (012/AC-2) | before (`{date: count}` for D+1 and D+7), created, target, min — `actor_type = "SYSTEM"`; emitted by `ensure_generalist_floor()` **only when it actually created slots** (a no-op inventory check emits nothing). `ensure_generalist_floor()` is evaluated lazily from the operator queue poll (`GET /operator/conversations`), never from a resolver / draft / anonymous-customer path — it creates slots in response to inventory decay, never in response to a query (`specs/004` clarification item 6 preserved; `DECISIONS.md` D-044). Not conversation-scoped |
...
| **`booking_script.autonomous_message_sent`** ⚠ (dynamic appointment availability, AA-10, Constitution Amendment 1.1.0) | conversation_id, message_id, step — **never** the customer's raw CPF or payment-question reply, and never the sent message's own body. `actor_type = "SYSTEM"` — one of exactly two event types in the catalog marking a message sent with no operator click (the other is `autonomy.message_sent` below, Amendment 1.2.0). `SELECT * FROM audit_events WHERE event_type IN ('booking_script.autonomous_message_sent', 'autonomy.message_sent')` is, by construction, a complete and exhaustive list of every autonomously-sent message in the system. |
| **`autonomy.message_sent`** ⚠ (010, Constitution Amendment 1.2.0; 011, Amendment 1.3.0 reuses the same event type) | conversation_id, message_id, pending_autonomous_send_id, generation_id, category (`null` for an 011/N5-mechanism send), mechanism (`'governed_autonomy'` or `'ungoverned_n5'`, added 011) — **never** the sent message's own body (already covered by `ai.draft_generated`/`ai.draft_accepted` at generation/composition time). `actor_type = "SYSTEM"` — see `booking_script.autonomous_message_sent`'s own note above for the exhaustive-query pattern covering all three mechanisms (N5 sends still use this same event type, distinguished only by the `mechanism` field — no new event type was needed). |
| **`autonomy.category_policy_changed`** (010) | category, before, after; `actor_id` = the operator who toggled it |
| **`autonomy.kill_switch_toggled`** (010) | before, after; `actor_id` = the operator who toggled it |
| **`autonomy.window_duration_changed`** (010) | before_seconds, after_seconds; `actor_id` = the operator who changed it |
| **`autonomy.pending_send_paused`** (010) | pending_autonomous_send_id; `actor_id` = the operator who paused it |
| **`autonomy.n5_kill_switch_toggled`** (011, Constitution Amendment 1.3.0) | before, after; `actor_id` = the operator who toggled it — independent of `autonomy.kill_switch_toggled` above |
| **`autonomy.idle_seconds_changed`** (011) | before_seconds, after_seconds; `actor_id` = the operator who changed it — shared by both the governed (010) and ungoverned (011) trigger-evaluation entry point |
| **`autonomy.n5_ungoverned_reply_generated`** (011) | ai_generation_id, prior_generation_id; `actor_type = "SYSTEM"` — recorded by `generate_ungoverned_reply()` itself, before the send it may lead to |
